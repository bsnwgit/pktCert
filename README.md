...
`GET /api/nav/manifest` (`app/api/nav.py`) publishes pktCert's own left-nav so
...
Selecting one of those rows opens pktCert's **real page** inside pktHub —
...
**pktCert** is one of ten apps in the pkt suite — self-hosted tooling for network
...
| **pktCert** *(you are here)* | `8763` | TLS certificate discovery and expiry tracking, plus an internal CA — issue, revoke and serve CRLs |
